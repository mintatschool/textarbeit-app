...
import { ConjugationTable, Tense, VerbDefinition } from '../types';

// Diese Datenbank ermöglicht Offline-Funktionalität.

type VerbDB = {
  [lemma: string]: {
    [key in Tense]?: ConjugationTable;
  };
};

// Hilfsfunktion für regelmäßige Verben
const createRegularVerb = (stem: string): Record<Tense, ConjugationTable> => {
    return {
...
const createSeparableVerb = (prefix: string, stem: string, infinitiveStem: string): Record<Tense, ConjugationTable> => {
    return {
...
export const COMMON_VERBS_DB: VerbDB = {
  // --- A ---
  ...
  'beginnen': { // Stark
    ...
    [Tense.FUTUR_I]: { ich: 'werde begonnen', du: 'wirst begonnen', er_sie_es: 'wird begonnen', wir: 'werden begonnen', ihr: 'werdet begonnen', sie_Sie: 'werden begonnen' },
  },
  'bekommen': { // Stark
  ...
  'lesen': {
    ...
    [Tense.FUTUR_I]: { ich: 'werde gelesen', du: 'wirst gelesen', er_sie_es: 'wird gelesen', wir: 'werden gelesen', ihr: 'werdet gelesen', sie_Sie: 'werden gelesen' },
  },
  ...
  // --- V ---
  'vergessen': {
    [Tense.PRAESENS]: { ich: 'vergesse', du: 'gisst', er_sie_es: 'gisst', wir: 'vergessen', ihr: 'vergesst', sie_Sie: 'vergessen' },
    [Tense.PRAETERITUM]: { ich: 'gaß', du: 'gaßest', er_sie_es: 'gaß', wir: 'gaßen', ihr: 'gaßt', sie_Sie: 'gaßen' },
    [Tense.PERFEKT]: { ich: 'habe gessen', du: 'hast gessen', er_sie_es: 'hat gessen', wir: 'haben gessen', ihr: 'habt gessen', sie_Sie: 'haben gessen' },
    [Tense.PLUSQUAMPERFEKT]: { ich: 'hatte gessen', du: 'hattest gessen', er_sie_es: 'hatte gessen', wir: 'hatten gessen', ihr: 'hattet gessen', sie_Sie: 'hatten gessen' },
    [Tense.FUTUR_I]: { ich: 'werde gessen', du: 'wirst gessen', er_sie_es: 'wird gessen', wir: 'werden gessen', ihr: 'werdet gessen', sie_Sie: 'werden gessen' },
  },
...
export const getLocalConjugation = (lemma: string, tense: Tense | string): ConjugationTable | null => {
  const lemmaLower = lemma.toLowerCase();
  // @ts-ignore - Index access with string/enum mix
  return COMMON_VERBS_DB[lemmaLower]?.[tense] || null;
};

export const isVerbInLocalDB = (lemma: string): boolean => {
  return Object.prototype.hasOwnProperty.call(COMMON_VERBS_DB, lemma.toLowerCase());
};

// --- NEU: Reverse Lookup Engine für Offline-Analyse ---

// Map: "konjugiertes_wort" -> { lemma: "grundform", tense: "zeitform", persons: Set<string> }
let REVERSE_VERB_INDEX: Map<string, { lemma: string, tense: Tense, persons: Set<string> }> | null = null;

const buildReverseIndex = () => {
  if (REVERSE_VERB_INDEX && REVERSE_VERB_INDEX.size > 0) return;
  ...
  Object.keys(COMMON_VERBS_DB).forEach(lemma => {
    ...
    Object.keys(verbData).forEach(tenseKey => {
      ...
      Object.entries(conjugations).forEach(([personKey, conjugatedForm]) => {
        ...
        // Füge Wort in Index ein oder aktualisiere Personen
        if (!REVERSE_VERB_INDEX!.has(key)) {
           REVERSE_VERB_INDEX!.set(key, { lemma, tense, persons: new Set([personKey]) });
        } else {
           const entry = REVERSE_VERB_INDEX!.get(key);
           if (entry && entry.lemma === lemma && entry.tense === tense) {
               entry.persons.add(personKey);
           }
        }
        
        // Zusätzlich bei zusammengesetzten Zeiten ("habe gemacht") das Partizip ("gemacht")
        if (key.includes(' ')) {
           const parts = key.split(' ');
           const participle = parts[parts.length - 1]; // Letztes Wort
           if (participle.length > 3) {
                if (!REVERSE_VERB_INDEX!.has(participle)) {
                    REVERSE_VERB_INDEX!.set(participle, { lemma, tense: Tense.PERFEKT, persons: new Set(['partizip']) }); 
                }
           }
        }
      });
  ...
  Object.keys(COMMON_VERBS_DB).forEach(lemma => {
      ...
      if (!REVERSE_VERB_INDEX!.has(lemmaLower)) {
          REVERSE_VERB_INDEX!.set(lemmaLower, { lemma, tense: Tense.PRAESENS, persons: new Set(['wir', 'sie_Sie']) });
      }
  });
};

/**
 * Versucht, den Text OHNE API zu analysieren, indem Wörter gegen die Datenbank geprüft werden.
 */
export const analyzeTextLocal = (text: string): VerbDefinition[] => {
  buildReverseIndex();
  
  const foundVerbs: VerbDefinition[] = [];
  
  // Verbesserte Tokenisierung: Nicht-Buchstaben entfernen, aber Umlaute behalten
  ...
  words.forEach((word, index) => {
    ...
    // Wenn nicht gefunden, einfache Heuristik für Endungen (z.B. "gingst" -> "ging")
    if (!match && cleanWord.endsWith('st')) {
         match = REVERSE_VERB_INDEX?.get(cleanWord.slice(0, -2));
    }
    
    if (match) {
      const personsArray = Array.from(match.persons).filter(p => p !== 'partizip');
      let personStr = personsArray.join('/');
      
      // Formatting for display
      personStr = personStr.replace('er_sie_es', 'er/sie/es').replace('sie_Sie', 'sie/Sie');

      foundVerbs.push({
        original: word,
        lemma: match.lemma,
        detectedTense: match.tense,
        detectedPerson: personStr,
        positionIndex: index
      });
    }
  });
...
// Initialize index immediately but asynchronously to prevent UI freeze on interaction
setTimeout(buildReverseIndex, 200);
