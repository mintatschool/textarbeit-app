            ...
            {showReward && (
                <div className="fixed inset-0 z-[200] pointer-events-none flex items-center justify-center">
                    {Array.from({ length: 30 }).map((_, i) => (
                        <div key={i} className="confetti" style={{ left: `${Math.random() * 100}%`, top: `-10%`, backgroundColor: ['#f00', '#0f0', '#00f', '#ff0', '#0ff', '#f0f'][Math.floor(Math.random() * 6)], animationDuration: `${2 + Math.random() * 3}s`, animationDelay: `${Math.random()}s` }}></div>
                    ))}
                    <div className="bg-white/95 backdrop-blur rounded-2xl p-10 shadow-2xl pop-animate pointer-events-auto text-center border-4 border-yellow-400 max-w-sm">
                        <Icons.CheckCircle size={64} className="text-green-500 mx-auto mb-4" />
                        <h2 className="text-3xl font-bold text-slate-800 mb-2">Perfekt! 🎉</h2>
                        <p className="text-slate-600 mb-8">Alle Wörter sind korrekt geschrieben.</p>
                        <button onClick={onClose} className="w-full py-4 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 shadow-lg transition-all active:scale-95">
