...
export const Space = React.memo(({ id, content, index, color, onMouseDown, onMouseEnter, isTextMarkerMode, isReadingMode, wordSpacing = 0, fontSize, colorPalette, letterSpacing = 0 }) => {
    ...
    return (
        <span
            ...
            className={`select-none inline-block whitespace-pre transition-colors duration-200 ${isReadingMode ? 'cursor-default' : (showMarkerLayout ? 'cursor-text' : 'cursor-default')}`}
            style={{
    ...
    return prev.color === next.color && prev.content === next.content && prev.index === next.index && prev.wordSpacing === next.wordSpacing && prev.fontSize === next.fontSize && prev.isTextMarkerMode === next.isTextMarkerMode && prev.colorPalette === next.colorPalette && prev.isReadingMode === next.isReadingMode && prev.letterSpacing === next.letterSpacing;
});
